...
def get_workflow_id_by_filename(owner, repo_name, workflow_filename):
    ...
    for workflow in workflows:
        if (
            workflow["path"].lower() == workflow_filename.lower()
...
def get_repo_collaborators(owner, repo_name):
    ...
    return [
        ...
        if collab["login"] not in ["geritwagner", "digital-work-labot"]
    ]
...
def process_repo(repo_data):
    ...
    # Note : the following does not (yet?) work:
...
def main():
    ...
    for repo in repos:
        if repo["html_url"] in [
            "https://github.com/digital-work-lab/digital-work-lab.github.io"
        ]:
            print(f"Skipping {repo['name']}")
            continue
        workflow_id = get_workflow_id_by_filename(
            ORG_NAME, repo["name"], workflow_filename
        )
        ...
        if not (
            ...
            or "teaching-materials" in repo_data["topics"]
        ):
